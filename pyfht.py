...
def sub_fht(n, m, seed=0, ordering=None):
    ...
    order: optional min(m, n)-long array of row indices to implement
           subsampling; generated by seed if not specified, but may be given
           to speed up subsequent runs on the same matrix. Do not specify 0.
    ...
    u = max(m, n)
    v = min(m, n)
    w = 2**int(np.ceil(np.log2(u)))

    if ordering is not None:
        assert ordering.shape == (v,)
    else:
        rng = np.random.RandomState(seed)
        idxs = np.arange(1, u, dtype=np.uint32)
        rng.shuffle(idxs)
        ordering = idxs[:v]

    def transform_subsample(x):
        assert x.size == u, "input must be max(m,n) long"
        y = np.zeros(w)
        y[:u] = x.reshape(u)
        _fht(w, y)
        return y[ordering]
    ...
    def embed_transform(x):
        assert x.size == v, "input must be min(m,n) long"
        y = np.zeros(w)
        y[ordering] = x
        _fht(w, y)
        return y[:u]
...
def block_sub_fht(n, m, l, seed=0, ordering=None):
    ...
    if ordering is not None:
        assert ordering.shape == (l, v)
    else:
        rng = np.random.RandomState(seed)
        ordering = np.empty((l, v), dtype=np.uint32)
        idxs = np.arange(1, u, dtype=np.uint32)
        for ll in range(l):
            rng.shuffle(idxs)
            ordering[ll] = idxs[:v]
